...
def place_tree_greedy(
    ...
    3. Back up in fine steps until no collision
    ...
    for _ in range(config.num_placement_attempts):
        ...
        while radius >= 0:
            ...
            # Check collision
            if tree_index:
                has_coll = has_collision_strtree(candidate_poly, tree_index, existing_polys)
            else:
                has_coll = has_collision(candidate_poly, existing_polys)
...
def simulated_annealing(
    ...
    """
    Optimize placements using Simulated Annealing.
    """
    n = len(placements)
    if n <= 1:
        return placements
...
class PackingSolver:
    ...
    def solve_single(self, n: int, verbose: bool = False) -> Solution:
        ...
        if overlaps:
            # Something went wrong, use unoptimized version
            solution = center_placements(prev_solution + [new_placement])
